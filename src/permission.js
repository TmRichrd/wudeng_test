...
router.beforeEach((to, from, next) => {
  ...
  if (getToken('Token') && sessionStorage.getItem('userinfo')) {
    if (to.path === '/login') {
      next({ path: '/' })
      NProgress.done()
    } else {
      if (!store.getters.isAddMenu) {
        store.dispatch('GetRouter').then(() => {
          const addrouters = store.getters.addRouters
          console.log(addrouters)
          formatRouter(addrouters)
          addrouters.push({ path: '*', redirect: '/404', hidden: true })
          router.addRoutes(addrouters) // 动态添加可访问权限路由表
          store.commit('UPDATE_ISADDMENU', true)
          if (to.redirectedFrom) {
            router.push({ path: to.redirectedFrom })
            return
